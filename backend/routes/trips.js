...
router.post('/', authenticateToken, upload.single('cover'), async (req, res) => {
  ...
  const sql = `INSERT INTO trips (user_id, name, location, start_date, end_date, description, cover) VALUES (?, ?, ?, ?, ?, ?, ?)`;
  try {
    const [result] = await pool.query(sql, [user_id, name, location, start_date, end_date, description, cover]);
    res.status(201).json({ id: result.insertId, name, location, start_date, end_date, description, cover });
  } catch (err) {
...
router.put('/:id', async (req, res) => {
  const { name, location, start_date, end_date, description } = req.body;
  try {
    await pool.query('UPDATE trips SET name=?, location=?, start_date=?, end_date=?, description=? WHERE id=?',
      [name, location, start_date, end_date, description, req.params.id]
    );
